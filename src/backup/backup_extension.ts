...
function applyGroupMatchDecorations(
    ...
    for (const groupName of Object.keys(groupMatches)) {
        ...
        const decorations: vscode.DecorationOptions[] = matches.map(
            (range) => ({
                range,
...
function highlightMatches(): void {
    ...
    if (!reText) {
        outputChannel.appendLine(
            "Abort highlighting. No text in the regex file."
        );
    }
    ...
    while ((match = pattern.exec(searchText))) {
        if (i > maxMatches) {
            console.log(`Max matches of ${maxMatches} reached.`);
            outputChannel.appendLine(`Max matches of ${maxMatches} reached.`);
            break;
        }
        if (new Date().getTime() > startTime + 1000) {
            console.log(`Timeout of ${timeout} reached.`);
